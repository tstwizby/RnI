...
int diagnoseScriptBuffers(){
	DEBUG("Diagnosing script buffers...");
	DEBUG("Currently at depth %d.",scrDepth);
	for(int i=scrDepth;i>0;i--){
		DEBUG("Script at depth %d: %s (pos %d)",i,scr_names[nextScript[i]],nextPos[i]);
	}
	DEBUG("Root script: %s (pos %d)",scr_names[nextScript[0]],nextPos[0]);
}
...
int writeChar(TextBox* t){
	...
	if(ch==ctrl_space){
		...
		int fy = (ch/fonts[t->font].columns)*fonts[t->font].height;
...
int loadText(TextBox* t){
	...
	int len = 0;
	while(data[len]&&((data[len]>ctrl_max)||(data[len]==ctrl_line)||(data[len]==ctrl_space))){len++;}
	if(len==0){return 0;}
	if(len+t->buffer->len>TXTBOX_LEN_MAX){ERROR(-1,"Textbox buffer length exceeded.");}
	int i = 0;
	int offset = 0;
	int nline = 0;
	int cpos = t->cx;
	while(i<len){
		...
		if(cpos>0&&cpos+wordlen>(t->width-(t->margin))){
			t->buffer->data[t->buffer->len+i+offset] = ctrl_line;
			cpos = wordlen+t->margin;
			offset++;
		}else{cpos+=wordlen;}
		while(data[i]>ctrl_max){
			t->buffer->data[t->buffer->len+i+offset] = data[i];
			i++;
			if(i+offset>TXTBOX_LEN_MAX){ERROR(-1,"Textbox buffer length exceeded.");}
		}
		if(data[i]==ctrl_space){
			t->buffer->data[t->buffer->len+i+offset]=ctrl_space;
		...
		if(t->buffer->len+i+offset>TXTBOX_LEN_MAX){ERROR(-1,"Textbox buffer length exceeded.");}
	}
	
	t->buffer->len += len+offset;
	t->frame = t->speed;
	scrBuffer.pos+=i-1;
	//DEBUG("Wrote %d chars (%d)",t->buffer->len,t->buffer->pos);
	//conv_print(t->buffer->data,t->buffer->len);
	if(t->speed==0){writeAllText(t);}
	return len+offset;
}

//Adds text to the given textbox from the given source, adding line breaks where necessary and returning the final length of the text box or -1 if the buffer size was exceeded or a control character was encountered.
int addText(TextBox* t, uint8_t* data){
	int offset = t->buffer->len;
	int cpos = t->cx;
	int i = 0;
	while(data[i]>ctrl_max||data[i]==ctrl_space||data[i]==ctrl_line){
		if(offset>=TXTBOX_LEN_MAX){ERROR(-1,"Textbox buffer length exceeded.");}
		int wordlen = wordLength(t->font,data+i);
		if(cpos>0&&cpos+wordlen>(t->width-2*(t->margin))){
	...
	t->buffer->len = offset;
	t->frame = t->speed;
	//DEBUG("wrote %d chars, total length %d",i,offset);
	//conv_print(data,i);
	//conv_print(t->buffer,t->len);
	return offset;
}
...
int loadScript(int index){
	...
	if(index<0||index>=scr_count){ERROR(1,"Script index out of bounds (%d)",index);}
	memcpy(scrBuffer_buffer,scr_buffers[index],scripts[index].len);
	scrBuffer.len = scripts[index].len;
	scrBuffer_buffer[scripts[index].len] = ctrl_end;
	currentScript=index;
	status = RUNNING;
	DEBUG("Script '%s' loaded",scr_names[index]);
...
int loadFastTravel(int dest, int dist){
	...
	status = RUNNING;
	DEBUG("Fast travel to %s",loc_names[dest]);
	if(timequeue&&scrDepth<MAX_REC_DEPTH){
		nextScript[scrDepth] = timequeue;
		nextPos[scrDepth] = -1;
		scrDepth++;
		DEBUG("Adding time-based script '%s'",scr_names[timequeue]);
		timequeue = 0;
	}
}
...
int loadMenu(uint8_t* data){
	...
	if(data[0]>MAX_MENU_OPTIONS){ERROR(1,"Maximum number of menu options exceeded.");}
	int i = 1;
	int count = 0;
	while(data[i]!=ctrl_end){
		addText(&txtmenu,data+i);
		while(data[i]!=ctrl_delim){/*printf("<%c,%d>",unconvert(data[i]),data[i]);*/i++;}
		i++;
		int j = 0;
		while(data[i+j]!=ctrl_menu&&data[i+j]!=ctrl_end){
			if(data[i+j]==ctrl_func){/*printf("[%s] %d,",func_names[data[i+j+1]],func_arg_count[data[i+j+1]]);*/j+=func_arg_count[data[i+j+1]]+1;}
			j++;
		}
...
int selectMenu(){
	clearBuffer(&scrBuffer);
	DEBUG("Menu selected: %d",menuSelect);
	memcpy(scrBuffer_buffer,menBuffer[menuSelect],menLen[menuSelect]);
	//conv_script(menBuffer[menuSelect],menLen[menuSelect]);
...
int stopScript(){
	...
	if(scrDepth>0){
		DEBUG("Returning to script %d, depth %d, pos %d",nextScript[scrDepth-1],scrDepth-1,nextPos[scrDepth]);
...
DEBUG("executing %s",func_names[params[0]]);
	...
	switch(params[0]){
		...
		case f_run:
			if(scrDepth==MAX_REC_DEPTH){ERROR(2,"Maximum recurrence depth reached. Use more gotos and fewer runs.");}
		...
		case f_transition:
			location = params[1];
			DEBUG("Loading location %d %s",params[1],loc_names[params[1]]);
		...
		case f_setlocscript:
			DEBUG("Setting location '%s' data element '%s' to script %d",loc_names[params[1]],lde_names[params[2]],params[3]);
			switch(params[2]){
				case lde_enter:
				...
				case lde_right:
					...
					return 0;
				default:
					ERROR(-1,"Unrecognized location data element encountered (%d)",params[2]);
			}
		case f_setlocdest:
			if(params[1]>=loc_count){ERROR(-1,"Target location ID out of range");}
			if(params[2]>=dir_count){ERROR(-1,"Unrecognized direction");}
			if(params[3]>=loc_count){ERROR(-1,"Destination location ID out of range");}
			if(params[4]<0){ERROR(-1,"Negative distances not currently permitted");}
			DEBUG("Setting location '%s' destination '%s' to '%s' at distance '%d'",loc_names[params[1]],dir_names[params[2]],loc_names[params[3]],params[4]);
			switch(params[2]){
				case dir_up:
					locations[params[1]].up=params[3];
			...
			return -1;
		case f_wait:
			DEBUG("Waiting for %d frames...",params[1]);
			delay = params[1];
			status = WAITING;
			return 0;
		case f_setscene:
		...
		default:
			ERROR(-1,"Unrecognized function code encountered (%d)",params[0]);
	}
	return 0;
}
